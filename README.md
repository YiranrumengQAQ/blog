...
## 皮肤补丁 · Neo-Brutalism（新粗野主义）

全站默认皮肤已通过**补丁层**整体切换为 Neo-Brutalism：粗黑描边、零模糊硬投影、
全直角、高饱和撞色、按下位移交互。**旧的黏土样式一行都没有删**，两套皮肤可以随时互切。

### 补丁的构成（全部是新增文件/新增行，零删改）

| 文件 | 作用 |
| --- | --- |
| `assets/css/neo-brutalism.css` | 补丁本体。加载在所有旧样式**之后**，两层覆盖：① 重新定义 `--clay-*` 设计令牌（圆角→0、柔和黏土阴影→`Xpx Ypx 0` 硬投影、马卡龙色→高饱和撞色、弹性过渡→脆快过渡）；② 对旧规则写死的地方（`border: none`、50% 圆角、渐变底色）逐条补上墨边和直角 |
| `assets/js/ui/uipatch.js` | 补丁开关。渲染右下角悬浮的 `BRUTAL / CLAY` 按钮，选择记在 localStorage（键 `blog-ui`），并校准移动端地址栏颜色（旧 `theme.js` / `editor.js` 只认黏土配色） |
| `index.html` / `editor.html` | 各追加 1 行 `<link>`、1 段防闪烁内联脚本（默认补丁生效；保存过「黏土风」的用户首帧前补 `ui-clay` 类）、1 行 `<script>`。**没有任何旧行为被修改** |

### 两套皮肤怎么互切

- **界面切换**：右下角悬浮按钮（`BRUTAL` ⇄ `CLAY`），刷新后记住选择；
- **临时回退**：给 `<html>` 加 `ui-clay` 类即可「撕掉」补丁——补丁里所有覆盖规则
  都用 `html:not(.ui-clay)` 门控，类一加，旧黏土 UI 原样复活；
- **彻底卸载**：删掉两个 HTML 里引用 `neo-brutalism.css` 的那一行 `<link>` 即可，
  旧 CSS 完好无损。

### Neo-Brutalism 怎么做出来的

| 要点 | 做法 |
| --- | --- |
| 粗黑描边 | 卡片/面板/按钮 3px 实线墨边，小控件 2px（亮色墨 `#101010`，暗色反转为米白 `#F2EFE9`） |
| 硬投影 | `box-shadow: 5px 5px 0 墨色`，零模糊零扩散；层级完全靠偏移量区分 |
| 全直角 | `--clay-r-*` 全部归零，硬编码的 50% 圆角（回到顶部、空状态图标等）逐个补丁覆盖 |
| 撞色 | 电光蓝 `#2D5BFF` / 荧光粉 `#FF90E8` / 芥末黄 `#FFCB2E`（黄 = 高亮专用：表头、计数徽章、置顶卡、选中文字） |
| 按下位移 | 悬停 `-2px,-2px` + 投影加长；按下 `+3px,+3px` + 投影归零，像真的把按钮按进纸里 |
| 纸感底纹 | 奶油纸底 `#FAF3E3` + 点阵网格（`radial-gradient` 平铺），替代黏土的三团柔光 |

暗色不是简单换底：黑底上黑影子看不见，粗野主义暗色把**墨边和投影一起反色成米白**，
撞色保持高饱和，对比反而更凶。

补丁同样尊重 `prefers-reduced-motion`，且禁用 JS 时补丁 CSS 照常生效
（只是没有切换按钮）。
