...
- **Claymorphism 黏土拟物风**：全站 UI 采用黏土质感设计——大圆角、马卡龙配色、
  「外投影 + 高光 + 内阴影」三层柔和阴影，几乎不用硬边框；卡片按下会「陷进去」，
  悬停会「浮起来」。亮/暗两套黏土配色，详见下文「设计系统」
...
- **安全渲染**：正文经 DOMPurify 消毒，误贴恶意代码也不会执行；白名单内的视频站 iframe（B 站 / YouTube / Vimeo 等）可正常嵌入，其余 iframe 一律删除
- **统一图标**：全站（阅读页 + 写作助手）共用一套内联 SVG 图标，不使用 emoji，颜色跟随主题
- **防主题闪烁**：`<head>` 内联脚本在首帧渲染前恢复上次的亮/暗主题，刷新不闪白
- **移动端体验**：抽屉菜单打开时锁定底层滚动（防穿透），布局高度使用 `dvh`（防地址栏遮挡），搜索结果关键词高亮
...
└── assets/
    ...
    │   │   ├── theme.js    # 亮暗主题
    │   │   ├── toast.js    # 轻提示
    │   │   ├── rain.js     # 天气引擎（Canvas 雨幕 + 屏幕冷凝水珠）
    │   │   └── glass.js    # 玻璃交互（指针光斑 + 卡片倾斜 + 入场显现）
    │   └── utils/
...
## 设计系统 · Claymorphism（黏土拟物风，底层）

> 当前默认外观是上文的「雨夜毛玻璃主题」（覆盖层），下面这套黏土是它的地基层：
> 玻璃主题通过重映射 `--clay-*` 令牌 + 逐组件覆盖实现，黏土文件本身一行没动。

全站视觉由 `assets/css/tokens.css` 一个文件驱动，**改这里就能全站换肤**，
不需要动任何组件样式。
...
| 要点 | 做法 | 对应令牌 |
| --- | --- | --- |
| 大圆角 | 卡片 30–38px、按钮全圆 | `--clay-r-sm` ~ `--clay-r-full` |
| 三层阴影 | 右下深影（沉）+ 左上白光（浮）+ 内阴影（厚） | `--clay-1` ~ `--clay-4` |
| 凹陷感 | 输入框 / 代码块 / 计数徽章用纯 inset 阴影 | `--clay-inset`、`--clay-inset-deep` |
| 有色底 | 页面底不能是纯白，否则黏土浮不起来 | `--clay-bg` + `--clay-bg-tint`（三团柔光） |
| 无硬边框 | 全站 `border: none`，层级完全靠阴影区分 | — |
| 弹性动效 | 过渡曲线带回弹，像捏黏土 | `--clay-t-fast` / `--clay-t` / `--clay-t-slow` |

### 配色

| 角色 | 亮色 | 说明 |
| --- | --- | --- |
| `--clay-primary` | `#7C6CF0` 薰衣草紫 | 主按钮 / 链接 / 激活态 |
| `--clay-secondary` | `#FF9BB3` 草莓奶昔粉 | 置顶标记 / 引用块 |
| `--clay-tertiary` | `#57D6C0` 薄荷绿 | 成功提示 / 次要按钮 |
| `--clay-bg` | `#EDEBFA` | 页面底色（暗色 `#201C2E`） |
| `--clay-surface` | `#F4F2FF` | 抬起的黏土块 |
| `--clay-surface-2` | `#E9E6FA` | 凹陷的黏土槽 |

**想换主题色**？只改 `--clay-primary` / `--clay-secondary` / `--clay-tertiary`
和对应的 `-soft` / `on-` 变量即可，阴影会自动跟着走。
换背景色时记得同步改 `index.html` / `editor.html` 里 `<meta name="theme-color">`
和 `assets/js/ui/theme.js`、`assets/js/editor.js` 里的两个十六进制值（移动端状态栏颜色）。

### 直接复用黏土质感

任意元素加工具类即可（定义在 `base.css`）：

```html
<div class="clay">凸起的黏土块</div>
<div class="clay-inset">凹进去的黏土槽</div>
```

### 兼容说明

`tokens.css` 底部保留了一整套 `--md-sys-*` 旧变量名，全部映射到黏土令牌。
这样即使有第三方片段 / 老代码还在用旧变量，也能自动拿到新配色，不会出现「半新半旧」。

暗色主题不是简单反色：高光透明度从 0.95 降到 0.07、深影加重到 0.62，
否则黏土在深色背景上会「发灰」失去体积感。

同时尊重系统的「减少动态效果」偏好（`prefers-reduced-motion`），
开启后所有弹性动画会自动关闭。

## 主题 · 雨夜毛玻璃（真·玻璃拟态 + 下雨）

全站默认主题是**雨夜毛玻璃**：所有卡片/顶栏/侧栏/弹窗都是极透的真玻璃
（GPU 实时 `backdrop-filter` 折射 + 镜面高光 + 虹膜边缘），背后下着三层雨，
屏幕玻璃上还挂着会滑落的冷凝水珠。**旧的黏土/粗野样式一行都没有删**，
只是在最后加了一层主题覆盖，随时可以换回来。

### 主题的构成

| 文件 | 作用 |
| --- | --- |
| `assets/css/glass-rain.css` | 主题本体。加载在所有旧样式**之后**：① 定义玻璃光学变量（`--glass-spec/--glass-sheen/--glass-iris`、6 层内叠阴影、18~30px 真模糊）；② 把 `--clay-*` 旧令牌重映射到玻璃；③ 逐组件覆盖为玻璃（顶栏/侧栏/卡片/文章/编辑器/灯箱/Toast……）；④ 亮/暗两套天气（雾雨白昼 / 雨夜霓虹） |
| `assets/js/ui/rain.js` | 天气引擎。三层 Canvas：动态天空（漂移云 + 城市光斑 + 游动高光 + 偶发闪电）、远景细雨、近景粗雨（落地涟漪）；外加屏幕玻璃冷凝水珠——每个水珠都是 `backdrop-filter` 实时透镜，会吞噬、滑落、留痕。默认开启，偏好记在 localStorage（键 `blog-rain`） |
| `assets/js/ui/glass.js` | 玻璃交互：跟随指针的柔光反光、卡片 3D 倾斜视差、卡片入场显现（带多重兜底，保证文字一定可见） |
| `index.html` / `editor.html` | 各追加 1 行 `<link>`、1 段雨效防闪烁内联脚本、顶栏 1 枚雨滴开关按钮（SVG 图标，按 `R` 也能开关）、2 行 `<script>` |

### 真玻璃是怎么算出来的

| 要点 | 做法 |
| --- | --- |
| 实时折射 | 卡片 `backdrop-filter: saturate() brightness() blur(18~30px)`，浏览器 GPU 每帧重算，不是半透明色块 |
| 镜面高光 | `--glass-spec` 155° 多段镜面渐变 + 悬停时 `--glass-sheen` 斜向滑光扫过 |
| 虹膜边缘 | `::before` + mask 环形裁切，只在 2px 描边环上做 `blur(30px)` 强透镜 |
| 三层雨 | CSS 雨线打底 + Canvas 远雨（~400 条）+ Canvas 近雨（~150 条，落地碎成涟漪），风场实时摆动 |
| 冷凝水珠 | 前景 DOM 水珠群：微珠呼吸闪烁 → 静珠长大 → 大珠吞噬小珠 → 重力滑落 → 留下 2~3 秒蒸发的水痕，全是 `backdrop-filter` 真透镜 |
| 闪电 | 7~22 秒随机一击，Canvas 分叉电弧 + 全屏泛光，35% 概率双重闪 |
| GPU 默认开 | 所有动画走 `translate3d / opacity` 合成层，Canvas 按 DPR 渲染；性能换好看，不设上限 |

### 回退旧皮肤

- **临时回退**：删掉两个 HTML 里引用 `glass-rain.css` 的那一行 `<link>`、
  `rain.js` / `glass.js` 的两行 `<script>`，并去掉 `<body>` 的 `glass-theme` 类，
  旧 UI 原样复活；
- 仓库里还留着 `assets/css/neo-brutalism.css`（粗野补丁本体，已取消引用），
  想看旧皮肤可以把它换回来。
